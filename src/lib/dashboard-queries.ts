...
import { ActivityType, AssetStatus, DealStage, TaskStatus } from "@prisma/client";
import { prisma } from "./db";

const TODAY_TASK_LIMIT = 50;
const TODAY_ACTIVITY_LIMIT = 50;
const TODAY_DEAL_LIMIT = 20;
...
export type TodayDashboardData = {
  priorityTasks: TodayTaskItem[];
  upcomingActivities: TodayActivityItem[];
  topDeals: TodayDealItem[];
};
...
export async function getWorkspaceSnapshot() {
  try {
    ...
    console.error("Workspace snapshot fetch failed:", error);
    return { focusDeal: null, timeline: [], tasks: [], assets: [] };
  }
}
